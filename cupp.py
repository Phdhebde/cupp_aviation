...
#  [Author]
#
#  Muris Kurgas aka j0rgan and Maxence Haegman
...
def interactive():
    ...
    #### Further information for the plane project

    str_year = input("> Starting Year (YYYY): ").lower()
    while len(str_year) != 4:
        print("\r\n[-] You must enter 4 digit for the starting year ! ")
        str_year = input("> Starting Year (YYYY): ").lower()
    profile["str_year"] = str(str_year)
    profile["fav_plane"] = input('> Favorite Plane: ').lower().replace(' ', '')
    profile["fav_air"] = input("> Favorite Airline").lower().replace(' ', '')
    load_master = input("> Load-master Graduation Year (YYYY): ").lower()
    while len(load_master != 4):
        print("\r\n You must enter 4 digit for the graduation year !")
        load_master = input("> Load-master Graduation Year (YYYY): ").lower()
    profile["load_master"] = str(load_master)
    profile["load_sheet"] = input("> Load-Sheet Software Licensed").lower().replace(' ', '')
    print("\r\n")
...
def generate_wordlist_from_profile(profile):
    ...
    fav_planeup = profile["fav_plane"].title()
    fav_airup = profile["fav_air"].title()
    load_sheetup = profile["load_sheet"].title()
    ...
    rev_kidup = kidup[::-1]
    # reverse label
    rev_fav_plane = profile["fav_plane"][::-1]
    rev_fav_planeup = fav_planeup[::-1]
    rev_fav_air = profile["fav_air"][::-1]
    rev_fav_airup = fav_airup[::-1]
    ...
    rev_plane = [rev_fav_plane, rev_fav_planeup]
    rev_air = [rev_fav_air, rev_fav_airup]
    ...
    kombinaac = [profile["company"], companyup, profile["fav_plane"], fav_planeup, profile["fav_air"], fav_airup, profile["load_sheet"], load_sheetup]
    ...
    # Adding some specific combination

    kombi[22] = list(komb(kombinaac, profile['str_year']))
    kombi[22] += list(komb(kombinaac, profile['load_master']))
    kombi[23] = list(komb(rev_plane, profile["str_year"]))
    kombi[23] += list(komb(rev_plane), profile['load_master'])
    kombi[24] = list(komb(rev_air, profile['load_master']))
    kombi[24] += list(komb(rev_air, profile['str_year']))
